...
library(ggpubr)
library(ggplot2)
knitr::opts_chunk$set(echo=FALSE, warning = FALSE, message = FALSE, cache = TRUE, fig.pos = 'h')
```
...
I hypothesize that there will be increased abundance of the qPCR target SSU rDNA and therefore, mercury methylating taxa, Iron Reducing Bacteria, and Sulate Reducing Bacteria downstream of spill site due to stimulation by coal ash constituents present in the sediment. 
...
\begin{center}
  \includegraphics[width=400pt]{figure/map}
   \captionof{figure}[Google Earth image of Dan River and sampling locations.]{Google Earth image of Dan River and sampling sites. Markers indicate samping sites: Blue, upstream of spill site; Green, leaching site; Yellow, downstream dredged locations; Orange, downstream not dredged.}\label{fig:map}
\end{center}
...
The Kuskall-Wallis test indicated a significant difference between the abundance of amplified SSU rDNA and sampling locations for each qPCR target assessed (Kruskall-Wallis p.value <0.05). For each target, sampling site D-01, the first sampling site downstream (0.3 km) from the spill site, indicated an elevated signal compared to some sites downstream and upstream (Figures \@ref(fig:hgcaomp) - \@ref(fig:srbomp), Table \@ref(tab:compare)). However, D-01 was not the location with the highest abundance. The sampling location D-03, the farthest upstream location, 3.6 km upstream of the spill, and 0.5 km downstream from the Smith River confluence, measured the hightest abundance of SSU rDNA of each of the targets tested (Figures \@ref(fig:hgcaomp) - \@ref(fig:srbomp), Table \@ref(tab:compareU)). 

Additionally, the leaching site, L-01 is not significantly different from the D-01 location. When comparing the leaching site to all others, targets are of a slightly higher abundance than most of the other sampling locations. This pattern is similar to the D-01 comparisons, but overall, less pairs are significantly different (Table \@ref(tab:compareL)). The same pattern was observed between the data normalized to the mass of sediment in each sample as well as the amount of organic matter present.    
...
\begin{center}
  \includegraphics[width=400pt]{figure/hgcaomp}
   \captionof{figure}[HgcA abundance normalized to organic matter at each sampling location.]{HgcA abundance normalized to organic matter at each sampling location. Asterisks signify difference between the reference, D-01 using the Wilcoxon Rank Sum test. * : p <= 0.05, ** : p <= 0.01, *** : p <= 0.001}\label{fig:hgcaomp}
\end{center}
...
The SSU rDNA abundance at the sampling location 0.3 km downstream of the spill site, D-01, has an elevated signal over some locations upstream and downstream. However, the furthest upstream site measured the highest abundance of all locations. Additionally, the leaching site and D-01 were not significantly different, and also exhibited and elevated signal over some locations. This may signify that there is an impact from leaching of coal ash constituents. All primers tested exhibited the similar patterns of significance and abundance per location and may possibly be correlated due to an unknown confounding factor.

This field study was constrained by access with few boat ramps, and two dams. River gauge height also provided a logistical obstacle for repeated field sampling.  The leaching site was accessed after permission from Duke Energy from their onsite boat ramp.  
